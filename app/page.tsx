...
function RunnerMotion({ compact = false }: { compact?: boolean; label?: string }) {
  const frameW = compact ? 120 : 182;
  const frameH = compact ? 142 : 194;
  const imageH = compact ? 168 : 228;
  const sheetShift = -(frameW * 9);
  const restShift = -(frameW * 4);
  const frameYOffset = compact ? -12 : -16;

  return (
    <div
      className={`jr-motion relative overflow-hidden ${compact ? "h-[150px]" : "h-[214px] sm:h-[228px]"}`}
      style={{
        ...
        ["--jr-frame-y" as any]: `${frameYOffset}px`,
      }}
    >
      <style>{`
        @keyframes jrRunCycle {
          from { transform: translate3d(0, var(--jr-frame-y), 0); }
          to { transform: translate3d(var(--jr-sheet-shift), var(--jr-frame-y), 0); }
        }
        ...
        .jr-motion .jr-cycle-viewport {
          ...
          filter: drop-shadow(0 0 6px rgba(255,255,255,.09)) drop-shadow(0 0 14px rgba(255,255,255,.05));
        }
        .jr-motion .jr-cycle-strip {
          ...
          filter: grayscale(1) invert(1) contrast(1.5) brightness(1.08);
        ...
        @media (prefers-reduced-motion: reduce) {
          ...
          .jr-motion .jr-cycle-strip { animation: none !important; transform: translate3d(var(--jr-rest-shift), var(--jr-frame-y), 0); }
        }
      `}</style>
      ...
      <div className="pointer-events-none absolute inset-x-[18%] top-[56%] h-[2px] bg-gradient-to-r from-transparent via-zinc-400/12 to-transparent jr-speed-b" />
      ...
      <div className="pointer-events-none absolute inset-x-[12%] bottom-6 h-px bg-gradient-to-r from-transparent via-zinc-700/35 to-transparent" />
    </div>
  );
}
